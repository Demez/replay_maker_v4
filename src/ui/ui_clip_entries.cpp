...
void draw_replay_list( int size[ 2 ] )
{
	...
	bottom_area_height += ImGui::GetFrameHeightWithSpacing();      // enabled checkbox and delete button

	ImGui::PushFont( font::normal_bold, font::size + 2 );
	bottom_area_height += ImGui::GetFrameHeightWithSpacing();  // clip section title
	bottom_area_height += ImGui::GetFrameHeight();  // export text
	ImGui::PopFont();

	// output vid delete video button
	//bottom_area_height += ImGui::GetFrameHeightWithSpacing();
	//bottom_area_height += style.ItemSpacing.y * 1.f;      

	//if ( !ImGui::BeginChild( "##video_list", {}, ImGuiChildFlags_Border ) )
	if ( ImGui::BeginChild( "##clip_list", { -1, region_avail.y - bottom_area_height }, ImGuiChildFlags_Border ) )
	{
		//ImGui::EndChild();
		//ImGui::EndChild();
		//return;
